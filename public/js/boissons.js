document.addEventListener('DOMContentLoaded', () => {
    ...
    optionCards.forEach(card => {
        ...
        if (validateBtn) {
            validateBtn.addEventListener('click', () => {
                ...
                fetch('/save-boissons-session', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify([{
                        id: card.dataset.boissonId,
                        qty: qty
                    }])
                })
                .then(() => {
                    sessionStorage.setItem('boissonAdded', 'true');
                    if (typeof updateCartBadge === 'function') updateCartBadge();
                })
                .catch(() => {
                    alert('Erreur lors de la validation de la boisson.');
                });
            });
        }
    });
    ...
    nextBtn.addEventListener('click', () => {
        ...
        if (notValidatedCards.length > 0) {
            alert(
                "⚠️ Certaines boissons ont été sélectionnés mais ne sont pas validés.\n\n" +
    ...
    applyBtn.addEventListener('click', () => {
        ...
        optionCards.forEach(card => {
            const cardCategory = card.dataset.boissonCategoryId;
            card.style.display =
